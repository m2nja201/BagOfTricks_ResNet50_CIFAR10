...
def train(model, train_loader, test_loader, criterion, optimizer, total_epochs, device):
    ...
    for epoch in range(total_epochs):
        ...
        for inputs, labels in tqdm(train_loader):
            inputs, labels = inputs.to(device), labels.to(device)
            if np.random.rand() < 0.1:  # mixup을 적용할 확률
                inputs, targets_a, targets_b, lam = mixup_data(inputs, labels, 0)
                outputs = model(inputs)
            ...
            #outputs = model(inputs)    # mixup을 사용하지 않을 경우 사용
            
            optimizer.zero_grad()
            #loss = criterion(outputs, labels)      # mixup을 사용하지 않을 경우 사용
            loss.backward()
            optimizer.step()
        ...
        '''if epoch < 5:                # 초기 warm up이 필요한 경우 사용
